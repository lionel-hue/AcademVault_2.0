    ...
    return (
        <MainLayout>
            <div className="container mx-auto px-3 sm:px-4 py-4 md:py-6">
                ...
                <div className="flex overflow-x-auto pb-2 mb-6 md:mb-8 -mx-4 px-4">
                    <div className="flex gap-1 md:gap-2 min-w-max">
                        {tabs.map(tab => (
                            <button
                                key={tab.id}
                                onClick={() => setActiveTab(tab.id)}
                                className={`px-3 md:px-4 py-2 md:py-3 rounded-lg font-medium transition-all duration-300 whitespace-nowrap flex items-center gap-2 ${activeTab === tab.id ? 'bg-blue-600 text-white shadow-lg' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}
                            >
                    ...
                    <div className="lg:col-span-2">
                        ...
                        {activeTab === 'profile' && (
                            <div className="space-y-4 md:space-y-6">
                                <MobileCard>
                                    <div className="flex flex-col md:flex-row items-start md:items-center gap-4 md:gap-6">
                                        <div className="relative">
                                            <div className="w-24 h-24 md:w-32 md:h-32 rounded-full overflow-hidden border-4 border-gray-800">
                                                <img
                                        ...
                                        <div className="flex-1">
                                            {editMode ? (
                                                ...
                                                <>
                                                    <h2 className="text-xl md:text-2xl font-bold text-white mb-2">{profile?.name}</h2>
                                ...
                                <MobileCard>
                                    <h3 className="text-lg font-bold text-white mb-4">Academic Information</h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div>
                                            ...
                                            {editMode ? (
                                                ...
                                                <p className="text-white">{profile?.institution || 'Not specified'}</p>
                                            )}
                                        </div>
                                        <div>
                                            <label className="block text-sm font-medium text-gray-300 mb-2">
                                ...
                                <MobileCard>
                                    ...
                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
                                        <div className="text-center p-3 md:p-4 bg-gray-800/30 rounded-lg">
                                            <div className="text-xl md:text-2xl font-bold text-white mb-1">{stats?.documents || 0}</div>
                                            <div className="text-xs md:text-sm text-gray-400">Documents</div>
                                        </div>
                                        <div className="text-center p-3 md:p-4 bg-gray-800/30 rounded-lg">
                                            <div className="text-xl md:text-2xl font-bold text-white mb-1">{stats?.collections || 0}</div>
                                            <div className="text-xs md:text-sm text-gray-400">Collections</div>
                                        </div>
                                        <div className="text-center p-3 md:p-4 bg-gray-800/30 rounded-lg">
                                            <div className="text-xl md:text-2xl font-bold text-white mb-1">{stats?.friends || 0}</div>
                                            <div className="text-xs md:text-sm text-gray-400">Friends</div>
                                        </div>
                                        <div className="text-center p-3 md:p-4 bg-gray-800/30 rounded-lg">
                                            <div className="text-xl md:text-2xl font-bold text-white mb-1">{stats?.storage_used || '0 MB'}</div>
                                    ...
                                    <div className="space-y-3">
                                        {activities.length > 0 ? (
                                            activities.map((activity, index) => (
                                                <div key={`activity-${activity.id || index}`} className="flex items-start gap-3 p-3 bg-gray-800/20 rounded-lg">
                        ...
                        {activeTab === 'preferences' && preferences && (
                            <div className="space-y-4">
                                                ...
                                                {['light', 'dark', 'auto'].map(theme => (
                                                    <label key={theme} className="cursor-pointer">
                                                        <input
                                                            ...
                                                            className="sr-only"
                                                        />
                                                        <div className={`p-3 rounded-lg text-center capitalize ${preferences.theme === theme ? 'bg-blue-600 text-white' : 'bg-gray-800 text-gray-300 hover:bg-gray-700'}`}>
                                                            {theme}
                                                        </div>
                                                    </label>
                                                ))}
                                    ...
                                    <div className="space-y-3">
                                        {Object.entries(preferences.notification_settings || {}).map(([key, value]) => (
                                            <div key={key} className="flex items-center justify-between">
                                                <div>
                                                    <p className="text-white capitalize">{key.replace('_', ' ')}</p>
                                                    <p className="text-gray-400 text-sm">Receive notifications for {key.replace('_', ' ')}</p>
                                                </div>
                                ...
                                <MobileButton onClick={handleUpdatePreferences} variant="primary" className="w-full">
                                    <i className="fas fa-save mr-2"></i>
                                    Save Preferences
                                </MobileButton>
                            </div>
                                ...
                                <MobileCard>
                                    ...
                                    <div className="space-y-4">
                                        ...
                                        <MobileButton
                                            ...
                                            disabled={changingPassword || !passwordData.current_password || !passwordData.new_password || passwordData.new_password !== passwordData.new_password_confirmation}
                                            variant="primary"
                                            className="w-full"
                                        >
                                            {changingPassword ? (
                        ...
                        <MobileCard>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-white">{profile?.registration_date ? new Date(profile.registration_date).toLocaleDateString() : 'N/A'}</span>
                                </div>
                                <div className="flex justify-between items-center">
                                    <span className="text-gray-400">Email Verified</span>
                                    <span className={`${profile?.email_verified_at ? 'text-green-400' : 'text-red-400'}`}>
                        ...
                        <MobileCard>
                            ...
                            <div className="space-y-3">
                                ...
                                <div className="grid grid-cols-2 gap-3 pt-3 border-t border-gray-800">
                                    <div className="text-center">
                                        <div className="text-lg font-bold text-white">{stats?.documents || 0}</div>
                                        <div className="text-xs text-gray-400">Documents</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-lg font-bold text-white">{stats?.categories || 0}</div>
                                        <div className="text-xs text-gray-400">Categories</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-lg font-bold text-white">{stats?.collections || 0}</div>
                                        <div className="text-xs text-gray-400">Collections</div>
                                    </div>
                                    <div className="text-center">
                                        <div className="text-lg font-bold text-white">{stats?.friends || 0}</div>
                        ...
                        <MobileCard>
                            ...
                            <div className="space-y-2">
                                <button
                                    ...
                                    className="w-full flex items-center gap-3 p-3 bg-gray-800/30 hover:bg-gray-800/50 rounded-lg transition-colors"
                                >
                                ...
                                >
                                    ...
                                    <span className="text-white">My Collections</span>
                                </button>
                            </div>
                        </MobileCard>
                    </div>
